...
from isaacteleop.cloudxr.wss import parse_args, run as wss_run
...
async def _main_async() -> None:
    ...
    if not ready:
        if not runtime_proc.is_alive() and runtime_proc.exitcode != 0:
            sys.exit(runtime_proc.exitcode if runtime_proc.exitcode is not None else 1)
    ...
    try:
        args, cert_paths = parse_args(log_file_path=wss_log_path)
        await wss_run(args, cert_paths, stop_future=stop)
    finally:
        terminate_or_kill_runtime(runtime_proc)
    print("Stopped.")
